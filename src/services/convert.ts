...
import { ConvertInput } from "types";

export type GetConvertTokensResponseType = {
  tokens: {
    [key: string]: {
      address: string;
      decimals: number;
...
export type GetConvertQuoteResponseType = {
  id: string;
  ...
  tool: string; // the convert or exchanged used for this transaction
  toolDetails: {
    key: string;
    name: string;
    logoURI: string;
...
export const getConvertQuoteRequest = async ({
  input,
  fromAddress,
  controller,
}: {
  ...
  return await axios.get<GetConvertQuoteResponseType>(
...
export type GetConvertConnectionsResponseType = {
  connections: {
    fromChainId: number;
    toChainId: number;
...
export const getConvertConnectionsRequest = async ({
  fromChain,
  toChain,
  fromToken,
  toToken,
  ...
  return await axios.get<GetConvertConnectionsResponseType>(
...
export type ConvertTokenRequestParams = {
  to: string;
  data: string;
  value: string;
  tokenIn: string;
...
export const convertTokensRequest = async (
  params: ConvertTokenRequestParams,
  controller?: AbortController
) => {
